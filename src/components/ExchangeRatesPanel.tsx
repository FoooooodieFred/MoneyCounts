...
export function ExchangeRatesPanel({
  ...
  return (
    <Card className={className} id={id} data-section="exchange-rates">
      <Card.Header className="rate-section-bottom__header">
        <div>
          <p className="eyebrow">Exchange</p>
          <Card.Title>1 {baseCurrency} 的兑换汇率</Card.Title>
          <Card.Description>
            {source} · {new Date(updatedAt).toLocaleString("zh-CN")}
          </Card.Description>
        </div>
        <Button data-action="exchange-refresh" onPress={onRefresh}>
          汇率刷新
        </Button>
      </Card.Header>
      <Card.Content>
